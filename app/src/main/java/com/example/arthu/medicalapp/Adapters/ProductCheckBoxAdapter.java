...
public class ProductCheckBoxAdapter extends ArrayAdapter<Product> {

    List<Product> modelItems = null;
    Context context;
    Map<Product, Boolean> productMap;

    public ProductCheckBoxAdapter(Context context, List<Product> resource) {
        super(context, R.layout.activity_row,resource);
        this.context = context;
        this.modelItems = resource;
        productMap = new HashMap<Product, Boolean>(resource.size());
    }
    ...
    public Map<Product, Boolean> getItemChecked(){
        ...
        for (Map.Entry<Product,Boolean> item : productMap.entrySet()){
            if(item.getValue()){
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.checkBox.setText(modelItems.get(position).Name);
